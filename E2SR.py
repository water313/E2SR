...
class CFE(nn.Module):
    def __init__(self, k1=5, k2=7, dilation=3):
        ...
        # 1D卷积用于通道之间的依赖关系建模
        self.conv1 = nn.Conv1d(1, 1, kernel_size=k1, padding=(k1 - 1) // 2, bias=False)

        # 添加dilation并确保特征图大小不变
        self.conv2 = nn.Conv1d(1, 1, kernel_size=k2, padding=(k2 - 1) * dilation // 2, dilation=dilation,
                               bias=False)

        # 使用膨胀卷积，保持输出尺寸不变
        self.conv3 = nn.Conv1d(1, 1, kernel_size=1, padding=0, bias=False)

        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        y = self.avg_pool(x)  # b, c, 1, 1
        y = self.conv1(y.squeeze(-1).transpose(-1, -2))  # 对维度进行转置以适应1D卷积 b, c, 1 -> b, 1, c -> b, inc, c
        y = self.conv3(self.conv2(y))
        y = y.transpose(-1, -2).unsqueeze(-1)  # 恢复维度
        y = self.sigmoid(y)

        return x * y.expand_as(x)  # 将通道的权重扩展到每一个像素点，与输入点乘
...
class FEB(nn.Module):
    def __init__(self, n_feats):
        ...
        self.spatial_gating_unit = SCFE(n_feats)
...
class E2SR(nn.Module):
    def __init__(self, n_colors=103, n_feats=128, n_lkb=3, scale=4):
        ...
        self.conv = nn.Sequential(nn.Conv2d(n_colors, n_feats, 3, 1, 1, bias=True),
                                  nn.GELU())
        self.body = nn.Sequential(*[FEB(n_feats) for _ in range(n_lkb)])

        out_feats = n_colors * scale ** 2
        self.scale = nn.Parameter(torch.zeros((1, out_feats, 1, 1)), requires_grad=True)

        self.squeeze = nn.Conv2d(n_feats, 32, kernel_size=3, stride=1, padding=1, bias=True)
        self.expand = nn.Conv2d(32, out_feats, kernel_size=3, stride=1, padding=1, bias=True)
